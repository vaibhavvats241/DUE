...
export const RecordPaymentModal: React.FC<RecordPaymentModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSubmitPayment({
        fromUserId,
        fromUserName: fromMember ? fromMember.name.replace(' (You)', '') : 'Sender',
        toUserId,
        toUserName: toMember ? toMember.name.replace(' (You)', '') : 'Recipient',
        amountPaise,
  ...
  return (
    <div id="record-payment-modal" className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 p-0 sm:p-4 backdrop-blur-xs">
      <div className="w-full max-w-md rounded-t-3xl sm:rounded-2xl bg-white shadow-2xl flex flex-col max-h-[90vh] overflow-hidden text-slate-900 border border-slate-100 animate-in slide-in-from-bottom-6 sm:zoom-in-95 duration-200">
        
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 bg-slate-50/70">
          <div>
            <h3 className="text-base font-bold text-slate-900">Record Direct Payment</h3>
            <p className="text-xs text-slate-500">Settles or reduces accumulated dues</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-5 overflow-y-auto space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full text-xs p-2.5 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-slate-50/50"
            />
          </div>
          ...
          <div className="pt-2">
            <button
              ...
              className="w-full py-3 bg-slate-900 hover:bg-slate-800 active:scale-[0.99] text-white rounded-xl text-sm font-bold shadow-md transition disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {isSubmitting ? (
                <span>Recording...</span>
              ) : (
