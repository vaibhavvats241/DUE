...
import { Users, Plus, ChevronDown, Check, LogOut, Share2, Copy } from 'lucide-react';
      ...
      <div className="max-w-md mx-auto px-4 py-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              ...
              {showUserDropdown && (
                <div className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-2xl py-2 text-slate-900 z-40 border border-slate-100 animate-in fade-in zoom-in-95 duration-150">
                  <div className="px-4 py-3 border-b border-slate-100">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div className="truncate">
                        ...
                        <div className="text-[11px] text-slate-500 truncate">ID: {currentUser.id}</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-2">
                    <button
                      ...
                      className="w-full py-2 px-3 hover:bg-rose-50 text-rose-700 rounded-xl text-xs font-bold transition flex items-center gap-2"
                    >
        ...
        <div className="mt-2.5 pt-2 border-t border-slate-800/80 flex items-center justify-between">
          <div className="relative flex-1 mr-2">
              ...
              <div className="truncate flex-1">
                ...
                {activeGroup && (
                  <p className="text-[10px] text-slate-400">
                    {activeGroup.members.length} members &bull; Code: {activeGroup.inviteCode}
                  </p>
                )}
              </div>
            ...
            {showGroupDropdown && (
              <div className="absolute left-0 mt-1.5 w-64 bg-white rounded-xl shadow-xl py-2 text-slate-900 z-40 border border-slate-100">
                <div className="px-3 py-1 text-[11px] text-slate-500 font-semibold uppercase tracking-wider">
                  Your Friend Groups
                </div>
                <div className="max-h-48 overflow-y-auto py-1">
                  {groups.map((grp) => (
                    ...
                    >
                      <div>
                        <p className={`font-semibold ${activeGroup?.id === grp.id ? 'text-blue-600' : 'text-slate-800'}`}>
                          {grp.name}
                        </p>
                        <p className="text-[10px] text-slate-400">Invite Code: {grp.inviteCode}</p>
                      </div>
                      {activeGroup?.id === grp.id && <Check className="w-3.5 h-3.5 text-blue-600" />}
                    </button>
                  ))}
                </div>
                ...
                <div className="pt-2 px-2 border-t border-slate-100 mt-1">
                  <button
                    ...
                    className="w-full py-1.5 text-center text-xs font-bold text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition flex items-center justify-center gap-1"
                  >
          ...
          {activeGroup && (
            <button
              ...
              className="flex items-center gap-1.5 bg-slate-800 hover:bg-slate-700 active:scale-95 text-slate-300 text-[11px] font-mono font-medium px-2.5 py-1 rounded-lg border border-slate-700 transition shrink-0"
            >
              {copiedCode ? (
                <>
